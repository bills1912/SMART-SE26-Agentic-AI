  ...
  return (
    <div className="relative" ref={menuRef}>
      <button
        ...
        className="flex items-center gap-1 px-2 py-1 rounded hover:bg-orange-50 dark:hover:bg-gray-700 transition-colors duration-200"
      >
        {user.picture ? (
          <img
            src={user.picture}
            alt={user.name}
            className="w-6 h-6 rounded-full border border-orange-200 dark:border-gray-600"
          />
        ) : (
        ...
        <div className="hidden md:block text-left">
          <p className="text-xs font-medium text-gray-800 dark:text-white leading-tight">{user.name}</p>
        </div>
        <ChevronDown className={`h-3.5 w-3.5 text-gray-600 dark:text-gray-300 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
